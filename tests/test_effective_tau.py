...
from review_engine import Point, WireSegment
...
def test_lumped_tau_simple():
    ...
    # C = 0.2*10 + 0.1*2*(10.1) = 4.02 fF; tau = 10*4.02*1e-3 = 0.0402 ps
    tau = compute_effective_tau(
        segments=[seg], r_per_sq=0.1, c_per_um=0.2, method="lumped"
    )
    assert tau == pytest.approx(0.0402, rel=1e-3)
...
def test_lumped_tau_multi_segment_sum():
    ...
    # Each seg: R=5, C=2.02 (area 1 + fringe 1.02); total R=10, C=4.04
    # tau = 10*4.04*1e-3 = 0.0404 ps
    tau = compute_effective_tau(segs, 0.1, 0.2, method="lumped")
    assert tau == pytest.approx(0.0404, rel=1e-3)
...
def test_output_in_picoseconds():
    ...
    # R=10, C=40.2 (area 20 + fringe 20.2) -> tau = 10*40.2*1e-3 = 0.402 ps
    tau = compute_effective_tau([seg], 0.1, 0.2)
    assert tau == pytest.approx(0.402, rel=1e-3)
